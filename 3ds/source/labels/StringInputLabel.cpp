...
/* Construct the Label. */
StringInputLabel::StringInputLabel(const int LabelX, const int LabelY, const int MaxLength, const std::string &CurrentVal, const std::string &InputText) : X(LabelX), Y(LabelY), MaxLength(MaxLength), CurrentVal(CurrentVal), InputText(InputText) {
	std::string Tmp = "";
...
void StringInputLabel::Input() {
	std::unique_ptr<Keyboard> Overlay = std::make_unique<Keyboard>(this->MaxLength, this->CurrentVal, this->InputText);
