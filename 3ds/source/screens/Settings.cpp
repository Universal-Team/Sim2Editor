...
void Settings::SetDefaultPath() {
	if (this->Tab == SettingsTab::Configuration) {
		std::unique_ptr<FileSelection> Overlay = std::make_unique<FileSelection>(CFG->SAVPath(), std::vector<std::string>({ "/" }), "Select a Default SAVPath.", true, true);
...
void Settings::DrawCredits() const {
	...
	Gui::DrawStringCentered(0, 200, 0.4f, TEXT_COLOR, "For Researches, see 'https://wiki.universal-team.net/sims2-research/'.");
};
...
void Settings::DrawPages() const {
	...
	switch(this->Tab) {
		case SettingsTab::Configuration:
			...
			Gui::DrawString(30, this->Positions[2].Y + 5, 0.4f, TEXT_COLOR, "Create Backups on SAVLoad");
			GFX::DrawCheckbox(this->Positions[2].X, this->Positions[2].Y, CFG->CreateBackups());

			Gui::DrawString(30, this->Positions[3].Y + 5, 0.4f, TEXT_COLOR, "Default SAV Path");
