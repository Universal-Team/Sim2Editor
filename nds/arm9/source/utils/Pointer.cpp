...
/*
	...
	const uint32_t hHeld: The keysHeld() variable.
*/
void Pointer::ScrollHandle(const uint32_t hHeld) {
	if (hHeld & KEY_LEFT) {
		if (Pointer::X > 0) Pointer::X -= PTR_SPEED;
	}

	if (hHeld & KEY_RIGHT) {
		if (Pointer::X < (256 - PTR_X_SIZE)) Pointer::X += PTR_SPEED;
	}

	if (hHeld & KEY_UP) {
		if (Pointer::Y > 0) Pointer::Y -= PTR_SPEED;
	}

	if (hHeld & KEY_DOWN) {
		if (Pointer::Y < (192 - PTR_Y_SIZE)) Pointer::Y += PTR_SPEED;
	}
};
...
/*
    Set Pointer Position from two int16_t's, or the ButtonPos Struct.
*/
